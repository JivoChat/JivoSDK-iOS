...
public final class JVDisplayController: NSObject {
    ...
    private func define(text: String?, forElement elementIndex: Int) {
        if let element = JVDisplayElement.allCases.dropFirst(elementIndex).first {
            _define(text: text, forElement: element)
        }
    }
    ...
    private func define(color hex: String?, forElement elementIndex: Int) {
        if let element = JVDisplayElement.allCases.dropFirst(elementIndex).first {
    ...
    private func define(image name: String?, forElement elementIndex: Int) {
        if let element = JVDisplayElement.allCases.dropFirst(elementIndex).first {
    ...
    @objc(setExtraItemsForMenu:withCaptions:actionHandler:)
    private func setExtraItems(menu menuIndex: Int, captions: [String], handler: @escaping (Int) -> Void) {
