...
extension ClientEntity: JVDisplayable {
    ...
    func assignedAgent() -> AgentEntity? {
    ...
    var session: ClientSessionEntity? {
        switch integration {
        case .none:
    ...
    var customData: [ClientCustomFieldEntity] {
        if let allObjects = m_custom_data?.allObjects as? [ClientCustomFieldEntity] {
            return allObjects
        }
        else {
            return Array()
