...
extension ClientEntity {
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        ...
        if let c = change as? JVClientGeneralChange {
            ...
            m_chat_id = c.chatID?.jv_toInt64(.standard) ?? m_chat_id
            m_channel_id = Int64(c.channelID)
            m_channel_name = c.channelName ?? m_channel_name
            m_channel = context.object(ChannelEntity.self, primaryId: c.channelID)
            m_display_name = c.displayName ?? String()
            m_avatar_link = c.avatarURL ?? m_avatar_link
            m_comment = c.comment
            m_visits_number = c.visitsNumber?.jv_toInt16(.standard) ?? m_visits_number
            m_navigates_number = c.navigatesNumber?.jv_toInt16(.standard) ?? m_navigates_number
            ...
            if let channelID = c.channelID {
                ...
                m_channel = context.object(ChannelEntity.self, primaryId: channelID)
            }
            ...
            m_task = context.object(TaskEntity.self, primaryId: c.taskID)
        }
