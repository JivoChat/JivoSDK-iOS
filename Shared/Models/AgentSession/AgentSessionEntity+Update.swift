//
//  AgentSessionEntity+Update.swift
//  App
//
...
extension AgentSessionEntity {
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        ...
        if let c = change as? JVAgentSessionGeneralChange {
            ...
            e_channels.setSet(Set(context.upsert(of: ChannelEntity.self, with: c.channels)))
        }
        else if let c = change as? JVAgentSessionChannelUpdateChange {
            let oldChannels = Array(channels.filter { $0.ID != c.channel.ID })
            
            if let newChannel = context.upsert(of: ChannelEntity.self, with: c.channel) {
    ...
    private var e_channels: NSMutableSet {
        return mutableSetValue(forKeyPath: #keyPath(AgentSessionEntity.m_channels))
    }
}
