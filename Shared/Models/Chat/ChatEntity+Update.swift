...
extension ChatEntity {
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        ...
        if let c = change as? JVChatGeneralChange {
            ...
            if c.attendees.isEmpty {
                ...
                let attendees = context.insert(of: ChatAttendeeEntity.self, with: c.attendees)
                e_attendees.setSet(Set(attendees.filter { $0.agent != nil }))
            }
            
            m_client = context.upsert(of: ClientEntity.self, with: c.client)
            ...
            if let clientID = c.client?.ID {
                ...
                m_active_ring = context.upsert(of: MessageEntity.self, with: parsedActiveRing)
                ...
                m_active_ring = context.upsert(of: MessageEntity.self, with: c.activeRing)
            }
            
            let mePredicate = NSPredicate(format: "m_agent.m_session != null")
            let meAttendees = e_attendees.filtered(using: mePredicate)
            
            if let meAttendee = meAttendees.first as? ChatAttendeeEntity {
            ...
            if let attendee = context.insert(of: ChatAttendeeEntity.self, with: c.attendee) {
                m_attendee = attendee
            }
            ...
            let desiredMessage: MessageEntity?
            if let key = c.messageGlobalKey {
                let customId = JVDatabaseModelCustomId(key: key.key, value: key.value)
                desiredMessage = context.object(MessageEntity.self, customId: customId)
            }
    ...
    private var e_attendees: NSMutableSet {
        return mutableSetValue(forKeyPath: #keyPath(ChatEntity.m_attendees))
    }
    
    private var e_agents: NSMutableSet {
        return mutableSetValue(forKeyPath: #keyPath(ChatEntity.m_agents))
    }
    ...
    private func updateLastMessageIfNeeded(context: JVIDatabaseContext, change: JVMessageLocalChange?) {
        guard let message = m_last_message else {
            m_last_message = context.upsert(of: MessageEntity.self, with: change)
        ...
        if let _ = context.messageWithCallID(change.body?.callID) {
            m_last_message = context.update(of: MessageEntity.self, with: change.copy(ID: message.ID))
