...
extension AgentEntity {
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        ...
        if let c = change as? JVAgentGeneralChange {
            ...
            m_status = context.upsert(of: AgentStatusEntity.self, with: c.status)
            ...
            m_title = c.title
            m_worktime = context.upsert(of: AgentWorktimeEntity.self, with: c.worktime)
            m_is_working = c.isWorking ?? m_is_working
            m_session = context.upsert(of: AgentSessionEntity.self, with: c.session) ?? m_session
            ...
            m_worktime = context.upsert(of: AgentWorktimeEntity.self, with: c.worktimeChange)
        }
            ...
            if let key = c.messageGlobalKey {
                ...
                m_last_message = context.object(MessageEntity.self, customId: customId)
            }
            ...
            m_chat = context.object(ChatEntity.self, primaryId: c.chatID)
