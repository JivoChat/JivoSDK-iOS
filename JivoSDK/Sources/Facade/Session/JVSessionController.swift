...
public final class JVSessionController: NSObject {
    ...
    private func setPreferredServer(_ index: Int) {
        if index < JVSessionServer.allCases.endIndex {
            _setPreferredServer(JVSessionServer.allCases[index])
        }
        else {
    ...
    @objc(setupWidgetID:userToken:)
    private func setup(widgetID: String, userToken: String?) -> JVClient {
        if let userToken {
