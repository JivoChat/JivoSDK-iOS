...
class SdkJoint: ISdkJoint {
    ...
    var isDisplaying: Bool {
        guard let canvas = chatModuleJoint?.view,
              let navigator = canvas.navigationController
        else {
            return false
        }
        
        if let container = navigator.view, container.window == nil {
            journal {"Something wrong: there is a view, without attached window"}
            return false
        }
        else {
            return true
