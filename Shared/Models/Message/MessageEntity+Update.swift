...
extension MessageEntity {
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        ...
        func _adjustSender(type: String, ID: Int, body: JVMessageBodyGeneralChange?) {
            ...
            case "system" where MessageType.by(name: type) == .proactive:
                let m_sender_agent_value = context.agent(for: ID, provideDefault: true)
                if m_sender_agent != m_sender_agent_value {
                    m_sender_agent = m_sender_agent_value
                }
        ...
        func _adjustBotMeta(text: String) {
            ...
            if m_body == nil {
                m_body = context.insert(of: MessageBodyEntity.self, with: JVMessageBodyGeneralChange(json: JsonElement()))
            }
        ...
        if let c = change as? JVMessageEmptyChange {
            ...
            m_channel = c.widgetID.flatMap { context.upsert(of: ChannelEntity.self, with: JVChannelBlankChange(ID: $0)) }
            ...
            m_client = context.client(for: Int(m_client_id), needsDefault: false)
            m_chat_id = c.chatID.jv_toInt64(.standard)
            m_type = MessageType.message.exportableName()
            m_is_markdown = false
            m_text = c.text.jv_trimmed()
            m_media = context.insert(of: MessageMediaEntity.self, with: c.media, validOnly: true)
            m_referral_source = context.insert(of: ReferralSourceEntity.self, with: c.referralSource, validOnly: true)
            ...
            m_id = c.ID.jv_toInt64(.standard)
            m_date = m_date_freezed ? m_date : Date()
            m_channel = context.upsert(of: ChannelEntity.self, with: JVChannelBlankChange(ID: c.channelID))
            m_client_id = c.clientID.jv_toInt64(.standard)
            m_client = context.client(for: Int(m_client_id), needsDefault: false)
            m_chat_id = c.chatID.jv_toInt64(.standard)
            m_type = MessageType.message.exportableName()
            ...
            m_ordering_index = 1
            m_text = c.text.jv_trimmed()
            m_type = MessageType.system.exportableName()
            m_is_markdown = false
            ...
            case .regular:
                m_type = MessageType.message.exportableName()
            case .email:
                m_type = MessageType.email.exportableName()
            case .sms:
                m_type = MessageType.message.exportableName()
            case .whatsapp:
                m_type = MessageType.message.exportableName()
            case .comment:
                m_type = MessageType.comment.exportableName()
            }
            ...
            m_is_markdown = c.isMarkdown
            m_text = c.text
            m_sender_client = context.object(ClientEntity.self, primaryId: clientID)
            m_media = context.insert(of: MessageMediaEntity.self, with: c.media, validOnly: true)
            ...
            c.updates.forEach { update in
                ...
                case let .media(newValue):
                    let media = context.insert(of: MessageMediaEntity.self, with: newValue, validOnly: true)
...
class JVMessageExtendedGeneralChange: JVMessageBaseGeneralChange {
    ...
    open override var isValid: Bool {
        if MessageType.by(name: type) == .call, !(body?.isValidCall == true) {
            return false
        }
...
class JVMessageGeneralChange: JVMessageExtendedGeneralChange {
    ...
    override var isValid: Bool {
        if MessageType.by(name: type) == .message, media?.type == "conference", senderType == "agent" {
            return false
        }
...
final class JVMessageQuoteChange: JVMessageGeneralChange {
    ...
    func copy(nesting json: JsonElement) -> JVMessageQuoteChange {
        return JVMessageQuoteChange(
            ...
            chatID: chatID,
            widgetID: widgetID,
            type: MessageType.message.exportableName(),
            isMarkdown: isMarkdown,
            senderID: senderID,
            senderType: senderType,
            text: text,
...
fileprivate func validateMessage(senderType: String, type: String) -> Bool {
    switch MessageType.by(name: type) {
...
class JVSDKMessageOfflineChange: JVDatabaseModelChange {
    ...
    let date = Date()
    let type = MessageType.offline.exportableName()
...
class JVSDKMessageHelloChange: JVDatabaseModelChange {
    ...
    let date = Date()
    let type = MessageType.hello.exportableName()
    let content: JVMessageContent
