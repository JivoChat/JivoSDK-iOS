    ...
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        ...
        if let c = change as? JVArchiveHitGeneralChange {
            ...
            if let chatItem = c.chatItem {
                m_item = context.upsert(of: ArchiveHitChatItemEntity.self, with: chatItem)
                m_item?.b_hit = self
            }
            else if let callItem = c.callItem {
                m_item = context.upsert(of: ArchiveHitCallItemEntity.self, with: callItem)
                m_item?.b_hit = self
            }
        }
    }
