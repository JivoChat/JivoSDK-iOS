...
extension JVClientSession {
    func performApply(context: JVIDatabaseContext, environment: JVIDatabaseEnvironment, change: JVDatabaseModelChange) {
        if let c = change as? JVClientSessionGeneralChange {
            ...
            e_history.setSet(Set(context.insert(of: JVPage.self, with: c.history)))

            if let value = c.UTM {
                m_utm = context.insert(of: JVClientSessionUtm.self, with: value)
            }
            
            if let value = c.geo {
                m_geo = context.insert(of: JVClientSessionGeo.self, with: value)
            }
            
            if let value = c.chatStartPage {
                m_start_page = context.insert(of: JVPage.self, with: value)
            }
            
            if let value = c.currentPage {
                m_current_page = context.insert(of: JVPage.self, with: value)
            }
        }
    }
    
    private var e_history: NSMutableSet {
        return mutableSetValue(forKeyPath: #keyPath(JVClientSession.m_history))
    }
}
